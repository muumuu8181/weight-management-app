...
window.initWeightTab = () => {
    ...
    // 🔄 Phase 2: UnifiedSelector初期化（タイミング選択）
    try {
        if (typeof UnifiedSelector !== 'undefined') {
            window.timingSelector = new UnifiedSelector({
                containerId: 'timingButtons',
                hiddenInputId: 'selectedTiming', 
                prefix: 'タイミング',
                items: ['起床後', 'トイレ前', 'トイレ後', '入浴前', '入浴後', '食前', '食後'],
                onSelectionChange: (value) => {
                    WeightTab.selectedTimingValue = value;
                    log(`⏰ タイミング選択: ${value}`);
                }
            });
            log('✅ UnifiedSelector (タイミング) 初期化完了');
        }
    } catch (error) {
        log(`⚠️ UnifiedSelector初期化エラー: ${error.message} - 既存システム継続`);
    }
    ...
    if (typeof window.loadCustomItems === 'function') {
        ...
        log('⚠️ 共通のloadCustomItems関数が見つかりません - フォールバック実行');
        // フォールバック処理は既存の関数を維持
    }
...
window.saveWeightData = async () => {
    ...
    try {
        ...
        // 🎯 スマートエフェクト実行（JSON設定ベース）
        const saveButton = document.querySelector('.save-button');
        if (window.smartEffects && saveButton) {
            const isEdit = WeightTab.editingEntryId !== null;
            const actionName = isEdit ? 'edit' : 'save';
            window.smartEffects.trigger('weight', actionName, saveButton);
            log(`✨ スマートエフェクト実行完了: weight.${actionName}`);
        }
...
// 🔄 Phase 4: モード制御を共通機能に統一
// shared/components/mode-control.js の setMode を活用
// （独自実装を削除し、共通機能へ移行）

// 🔄 Phase 4: 対象選択も共通機能に統一
// shared/components/mode-control.js の selectTarget を活用

// 🔄 Phase 4: handleModeAction も共通機能に統一
// shared/components/mode-control.js に移行済み
